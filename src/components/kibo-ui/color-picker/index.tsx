...
export const ColorPicker = ({
  ...
  // Update internal HSL/alpha when controlled value changes
  useEffect(() => {
    if (value) {
      const c = Color(value);
      const [h, s, l] = c.hsl().array();
      const a = c.alpha();
      setHueState(Math.round(isFinite(h) ? h : 0));
      setSaturationState(Math.round(isFinite(s) ? s : 100));
      setLightnessState(Math.round(isFinite(l) ? l : 50));
      setAlphaState(Math.round(isFinite(a) ? a * 100 : 100));
    }
  }, [value]);
  ...
  const emitChange = useCallback(
    (
      ...
      const rgb = color
        ...
        .array()
        .map((v) => Math.round(v));
      const a = Math.round((nextAlpha / 100) * 1000) / 1000;
      const signature = `${rgb[0]},${rgb[1]},${rgb[2]},${a}`;
      if (signature === lastSignatureRef.current) return;
      lastSignatureRef.current = signature;
      onChange([rgb[0], rgb[1], rgb[2], a]);
    },
    [onChange]
  );
  ...
  const setHue = useCallback(
    (next: number) => {
      setHueState(next);
      emitChange(next, saturation, lightness, alpha);
    },
    [alpha, emitChange, lightness, saturation]
  );

  const setSaturation = useCallback(
    (next: number) => {
      setSaturationState(next);
      emitChange(hue, next, lightness, alpha);
    },
    [alpha, emitChange, hue, lightness]
  );

  const setLightness = useCallback(
    (next: number) => {
      setLightnessState(next);
      emitChange(hue, saturation, next, alpha);
    },
    [alpha, emitChange, hue, saturation]
  );

  const setAlpha = useCallback(
    (next: number) => {
      setAlphaState(next);
      emitChange(hue, saturation, lightness, next);
    },
    [emitChange, hue, lightness, saturation]
  );
  ...
  return (
    <Slider.Root
      ...
      onValueChange={([next]) => {
        ...
        if (typeof next === "number" && next !== lastHueRef.current) {
          lastHueRef.current = next;
          setHue(next);
        }
      }}
      step={1}
...
export const ColorPickerAlpha = ({
  ...
  const lastAlphaRef = useRef<number>(alpha);
  const isInteractingRef = useRef(false);
  ...
  return (
    <Slider.Root
      ...
      onValueChange={([next]) => {
        if (!isInteractingRef.current) return;
        if (typeof next === "number" && next !== lastAlphaRef.current) {
